...
from src.detectron2.detectron2_repo.detectron2.engine.defaults import DefaultPredictor
...
def forward_step(cfg,mode):
    ...
    print("Generating detected trajectories pose_gen.....")
    predictions = []
    for i,(imgs,meta) in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        # imgs = imgs.numpy()[::-1, :, :]
        # for j in range(imgs.shape[0]):
        #     prediction = predictor(imgs[j].transpose(1, 2, 0)) 
