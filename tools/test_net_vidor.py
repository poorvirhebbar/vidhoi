...
from slowfast.utils.view_annotations import save_pred_imgs
...
def perform_test(test_loader, model, cfg, writer=None):
    ...
    use_proposal = False
    skipped_idx = []
    skipped_frame = 0 # count of skipped frame because no human detected or only 1 human and no obj detected
    if cfg.DEMO.ENABLE:
    ...
    for inputs, meta in tqdm(test_loader):
        if cfg.DEMO.ENABLE:
            ...
            # poped from meta to avoid error at the of transfering to cuda
            meta.pop('orig_video_idx')
        ...
        # skip keyframe because no human detected or only 1 human and no obj detected
        if use_proposal and (0 not in meta['proposal_classes'][:, 1] or meta['proposal_lengths'][0])==1:
            skipped_idx.append(count)
            skipped_frame += 1
            continue
            #import pdb; pdb.set_trace()
        
        preds, action_labels, bbox_pair_ids, gt_bbox_pair_ids = model(inputs, meta["boxes"], meta['proposal_classes'], meta['proposal_lengths'], meta['action_labels'], meta['obj_classes'], meta['obj_classes_lengths'], trajectories=trajectories, human_poses=human_poses, trajectory_boxes=trajectory_boxes, skeleton_imgs=skeleton_imgs, trajectory_box_masks=trajectory_box_masks, heatmaps=heatmaps)
        ...
        results = {
            ...
            'ori_boxes': ori_boxes.detach().cpu().tolist(),
        ...
        '''
        #CREATE A NEW OPTION FOR THIS LATER#
        if not cfg.DEMO.ENABLE_ALL:
            all_results.append(results)
        
        if cfg.DEMO.ENABLE_ALL:
            #import pdb; pdb.set_trace()  
            save_pred_imgs(cfg, results,'val') 

        if cfg.DEMO.ENABLE_ALL and count == 1500:
            break
        '''
        count += 1
    ...
    print('SKIPPED IDX : ', skipped_idx)
    logger.info(f'SKIPPED FRAMES:{skipped_frame}')
    logger.info(f'SKIPPED IDX:{skipped_idx}')
