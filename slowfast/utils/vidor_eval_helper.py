...
def tune_vidor(
    thresh,
    all_preds,
    all_preds_score, # n_iters, n_pairs, 50
    ...
    for idx in tqdm(range(len(all_preds))): # for each keyframe
        ...
        # preds_scores = [
        #     ((all_preds_score[idx][i][j] if all_preds_score[idx][i][j] > 0 else 1e-300) + \
        #     (all_proposal_scores[idx][preds_bbox_pair_ids[i][0]][1] if all_proposal_scores[idx][preds_bbox_pair_ids[i][0]][1] > 0 else 1e-300) + \
        #     (all_proposal_scores[idx][preds_bbox_pair_ids[i][1]][1] if all_proposal_scores[idx][preds_bbox_pair_ids[i][1]][1] > 0 else 1e-300), i, j) 
        ...
        # preds_scores = preds_scores[:200] # 100
        
        at_least_one_pair_bbox_detected = False
       
        
        for i in range(len(all_preds_score[idx])): # for each HOI prediction, i-th pair and j-th action 
            labels = all_gt_action_labels[idx][i]

            is_match = False

            preds = [1 if all_preds_score[idx][i][j] > thresh else 0 for j in range(50)]

            pred_labels.append(preds)
            gt_labels.append(labels)

    # f1_scores = f1_score(gt_labels, pred_labels, average=None)
    ap_scores = average_precision_score(gt_labels, pred_labels, average=None)
...
def search_thresholds(
    ...
    thresholds = [0.001, 0.005, 0.01, 0.02, 0.03, 0.05, 0.07,  0.1, 0.12,  0.15, 0.18,  0.2, 0.25, 0.3, 0.5]
    #thresholds = [0.000001, 0.000005, 0.00001, 0.000025, 0.00005, 0.0001, 0.00025, 0.0005, 0.001, 0.0025, 0.005, 0.01]

    #all_map = np.zeros((557, len(thresholds)))
    all_map = np.zeros((50, len(thresholds)))
    for i, thresh in enumerate(thresholds):
        ...
        '''
        #import pdb; pdb.set_trace()
        all_map[:, i] = np.array(ap_score) # 50 x 11
    max_indices = all_map.argmax(1) # 50
    thresholds = np.array(thresholds)
    thresh_vec = thresholds[max_indices]
...
def search_thresholds(
    ...
    all_map = np.zeros((50, len(thresholds)))
    for i, thresh in enumerate(thresholds):
        _, _, _, _, _, ap_score = evaluate_vidor(
            all_preds,
            all_preds_score,
            all_preds_bbox_pair_ids,
            all_proposal_scores,
        ...
        all_map[:, i] = np.array(ap_score) # 557 x 11
    max_indices = all_map.argmax(1) # 557
    thresholds = np.array(thresholds)
    thresh_vec = thresholds[max_indices]

    return thresh_vec
'''
def compute_mAP(tp, fp, sum_gt, scores, mode='full'):
    # Compute mAP & recall
    with open(f'slowfast/datasets/vidor/{mode}_triplets.pkl','rb') as f:
        selected_triplets = pickle.load(f)

    ap = np.zeros(len(selected_triplets))
    wt_ap = np.zeros(len(selected_triplets))
    count = 0
    max_recall = np.zeros(len(selected_triplets))
    
    #for i, triplet_class in tqdm(enumerate(tp.keys())):
    # import pdb; pdb.set_trace()
    for i, triplet_class in tqdm(enumerate(selected_triplets)):
        #if triplet_class not in selected_triplets:
        #    continue
        ...
        wt_ap[i] = sum_gt_temp * ap[i]
        count += sum_gt_temp
        max_recall[i] = np.max(rec)
    
    import pdb; pdb.set_trace()
    mAP = np.mean(ap[:])
    wt_mAP = np.sum(wt_ap[:]) / count
    m_rec = np.mean(max_recall[:])

    logger.info(f'[Final] {mode} mAP: {mAP} ')
    logger.info(f'[Final] {mode} max recall: {m_rec}')
    
    print(f'{mode} mAP: {mAP} wt_mAP: {wt_mAP}')
    print(f'{mode} max recall: {m_rec}')

def analyze_fp_scores(fp, scores):
    
    score_thres = math.log(0.2)
    triplets_with_high_score = {}
    class_id = []
    count_list = []
    total_list = []
    ...
    for triplet in fp.keys():
        count = 0
        total = len(fp[triplet])
        for i,x in enumerate(fp[triplet]):
            if x and scores[triplet][i]>score_thres:
                count+=1
        if count/total > 0.5:
            high_occ.append(triplet)
        class_id.append(triplet)
        count_list.append(count)
        total_list.append(total)
        triplets_with_high_score[str(triplet)] = [count,total]
    print(len(high_occ))
    import pandas as pd
    df = pd.DataFrame({'class_id' : class_id,
                        'count' : count_list,
                        'total' : total_list})
    df.to_csv('fp_score_analysis.csv', index=False)
    import pdb; pdb.set_trace()    
    print('abc')
...
def evaluate_vidor(
    ...
    tp, fp, scores, sum_gt = {}, {}, {}, {}
    '''
    thresholds = [0.25 , 0.2  , 0.3  , 0.05 , 0.2  , 0.2  , 0.15 , 0.03 , 0.25 ,0.25 , 
                    0.3  , 0.3  , 0.5  , 0.01 , 0.01 , 0.5  , 0.03 , 0.3 , 0.02 , 0.07 , 0.3  , 
                    0.02 , 0.07 , 0.25 , 0.05 , 0.001, 0.18 , 0.5  , 0.5  , 0.001, 0.001, 0.07 , 0.05 , 
                    0.3  , 0.03 , 0.3  , 0.005, 0.15 , 0.03 , 0.2  , 0.12 , 0.5  , 0.001, 0.03 , 0.001, 0.001, 0.001, 0.001, 0.5  , 0.005]
    thresholds = [0.15 , 0.18 , 0.2  , 0.12 , 0.2  , 0.1  , 0.05 , 0.05 , 0.005, 0.3  , 0.12 , 0.001, 0.5  , 
            0.001, 0.001, 0.3  , 0.001, 0.3  , 0.01 , 0.01 , 0.05 , 0.001, 0.25 , 0.25 , 0.02 , 0.001, 0.01 , 
            0.3  , 0.12 , 0.001, 0.001, 0.001, 0.12 , 0.02 , 0.001, 0.25 , 0.03 , 0.01 , 0.07 , 0.5  , 0.07 , 0.25 , 0.001, 0.25 , 0.001, 0.001, 0.005, 0.001, 0.5  , 0.001]
    '''
    #w
    #thresholds = [0.5, 0.2, 0.5, 0.1, 0.25, 0.1, 0.12, 0.05, 0.18, 0.15, 0.3, 0.001, 0.5, 0.005, 0.005, 0.5, 0.01, 0.5, 0.01, 0.2, 0.12, 0.001, 0.25, 0.2, 0.03, 0.001, 0.001, 0.2, 0.07, 0.001, 0.001, 0.2, 0.02, 0.3, 0.12, 0.1, 0.02, 0.001, 0.12, 0.25, 0.05, 0.5, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.5, 0.001] 
    #v
    #thresholds = [0.5, 0.2, 0.25, 0.3, 0.25, 0.15, 0.12, 0.5, 0.03, 0.15, 0.25, 0.001, 0.5, 0.005, 0.001, 0.3, 0.01, 0.3, 0.02, 0.03, 0.18, 0.001, 0.1, 0.2, 0.02, 0.001, 0.001, 0.2, 0.03, 0.001, 0.001, 0.25, 0.001, 0.15, 0.001, 0.25, 0.001, 0.1, 0.01, 0.02, 0.12, 0.5, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.25, 0.001]
    #f
    #thresholds = [0.3, 0.3, 0.5, 0.12, 0.5,   0.3,   0.25,  0.12,  0.5,   0.3,   0.3,   0.18, 0.5,   0.05,  0.07,  0.3,   0.1,   0.5,   0.1,   0.15,  0.2,   0.02,  0.18,  0.3, 0.25,  0.1,   0.18,  0.3,   0.5,   0.005, 0.03,  0.12,  0.05,  0.25,  0.2,   0.3, 0.15,  0.12,  0.25,  0.3,   0.3,   0.18,  0.001, 0.03,  0.01,  0.25,  0.02,  0.001, 0.5, 0.07]
    #p
    #thresholds = [0.3,   0.25,  0.5,   0.5,   0.3,   0.2,   0.3,   0.3,   0.005, 0.25,  0.5,   0.001, 0.5,   0.005, 0.03,  0.2,   0.005, 0.5,   0.02,  0.2,   0.2,   0.001, 0.15,  0.5, 0.01,  0.001, 0.001, 0.5,   0.5,   0.001, 0.001, 0.18,  0.25,  0.5,   0.5,   0.5, 0.001, 0.01,  0.005, 0.25,  0.25,  0.02,  0.001, 0.5,   0.001, 0.001, 0.001, 0.001, 0.5,   0.001]
    #s
    thresholds = [0.3,   0.12,  0.2,   0.5,   0.15,  0.07,  0.05,  0.3,   0.001, 0.12,  0.01,  0.005, 0.05,  0.001, 0.001 ,0.25,  0.001, 0.18,  0.001, 0.001, 0.05,  0.001, 0.1,   0.001, 0.001, 0.001, 0.001, 0.1,   0.03,  0.001, 0.001, 0.001, 0.001, 0.005, 0.001, 0.005, 0.001, 0.03,  0.005, 0.001, 0.03,  0.03,  0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.1,   0.001]
    ...
    # delete triplet classes for rare, non-rare
    if mode == "rare" or mode == "non_rare":
        delete_less_than_25_instances = not (mode=="rare")
        delete_more_than_25_instances = not (mode=="non_rare")
    ...
    for idx in tqdm(range(len(all_preds))): # for each keyframe
        ...
        preds_scores.sort(reverse=True)
        # WARNING: EXPERIMENTAL CODE
        preds_scores = preds_scores[:100] # 100
        #import pdb; pdb.set_trace() 
        at_least_one_pair_bbox_detected = False
        ...
        for score, i, j in preds_scores: # for each HOI prediction, i-th pair and j-th action
            ...
            for k, gt_bbox_pair_id in enumerate(gt_bbox_pair_ids):  # for each ground truth HOI
                ...
                if gt_sub_cls == pred_sub_cls and gt_obj_cls == pred_obj_cls and sub_ov >= 0.5 and obj_ov >= 0.5:
                    ...
                    if gt_rel_cls == 1.0 and score > math.log(thresholds[j]):
                    #if gt_rel_cls == 1.0 and score > math.log(0.2):
                        is_match = True
                        correct_hoi_count += 1
    ...
    if mode == "full":
        with open(f'slowfast/datasets/vidor/rare_triplets.pkl','rb') as f:
            rare_triplets = pickle.load(f)
        with open(f'slowfast/datasets/vidor/non_rare_triplets.pkl','rb') as f:
            non_rare_triplets = pickle.load(f)
        # import pdb; pdb.set_trace()

        rare_ids, non_rare_ids = [], []
    for i, triplet_class in tqdm(enumerate(tp.keys())):
        if mode == "full":
            if triplet_class in rare_triplets:
                rare_ids.append(i)
            elif triplet_class in non_rare_triplets:
                non_rare_ids.append(i)
            else:
                print("BUG")
    ...
    mAP = np.mean(ap[:])
    if mode == "full":
        rare_mAP = np.mean(ap[rare_ids])
        non_rare_mAP = np.mean(ap[non_rare_ids])
    m_rec = np.mean(max_recall[:])

    logger.info(f'[Final] mAP: {mAP}')
    logger.info(f'[Final] max recall: {m_rec}')
    if mode == "full":
        print(f'[Final] Rare mAP: {rare_mAP}')
        print(f'[Final] Non Rare mAP: {non_rare_mAP}')
        
    # compute_mAP(tp, fp, sum_gt, scores, "rare")
    # compute_mAP(tp, fp, sum_gt, scores, "non_rare")
    
    # fp with high score
    #analyze_fp_scores(fp, scores)

    return mAP, m_rec, hd, dt, one_dr
    #return mAP, m_rec, hd, dt, one_dr, ap
    
def evaluate_decile_vidor(
    all_preds,
    all_preds_score,
    all_preds_bbox_pair_ids,
    all_proposal_scores,
        ...
        for score, i, j in preds_scores: # for each HOI prediction, i-th pair and j-th action
            ...
            for k, gt_bbox_pair_id in enumerate(gt_bbox_pair_ids):  # for each ground truth HOI
                ...
                if gt_sub_cls == pred_sub_cls and gt_obj_cls == pred_obj_cls and sub_ov >= 0.5 and obj_ov >= 0.5:
                    ...
                    # if gt_rel_cls == 1.0 and score > math.log(thresholds):
                    if gt_rel_cls == 1.0: #and score > math.log(0.5):
                        is_match = True
                        correct_hoi_count += 1
                        min_ov_cur = min(sub_ov, obj_ov)
                        if min_ov_cur > max_ov:
    ...
    if mode == "full":
        with open(f'slowfast/datasets/vidor/triplet_frequencies_full.pkl','rb') as f:
            triplet_frequencies = pickle.load(f)

        sorted_list = sorted(triplet_frequencies, key=triplet_frequencies.get)
