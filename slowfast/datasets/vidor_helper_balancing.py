...
def load_boxes_and_labels(cfg, mode):
    ...
    ann_filenames = [ #list of filenames of gt and predicted labels and boxes
        os.path.join(cfg.VIDOR.ANNOTATION_DIR, filename)
        for filename in gt_lists + pred_lists
    ]
    ann_is_gt_box = [True] * len(gt_lists)#  + [False] * len(pred_lists)
    ...
    for filename, is_gt_box in zip(ann_filenames, ann_is_gt_box):
        with PathManager.open(filename, "r") as f:
            anns = json.load(f)#has all annotations(entire {mode}_frame_annots.json file)
        
        while end_cur_frame_idx < len(anns):
            start_cur_frame_idx = end_cur_frame_idx
            all_obj_ids = set()
            while end_cur_frame_idx < len(anns) and anns[end_cur_frame_idx]['video_id'] == anns[start_cur_frame_idx]['video_id'] and anns[end_cur_frame_idx]['frame_id'] == anns[start_cur_frame_idx]['frame_id']:#for 1 frame all human-object interaction pairs
            ...
            if is_gt_box:#true for train/val gt boxes and labels file
            ...
            for ann in cur_frame_anns:#for each interaction in curr frame
                if is_gt_box:
...
def get_keyframe_data(cfg, instances, mode):
    ...
    Args:
        boxes_and_labels (list[dict]): a list which maps from video_idx to a dict.
            Each dict `frame_sec` to a list of boxes and corresponding labels. # every video_idx.. list of frames..every frame you have boxes and labels

    Returns:
        keyframe_indices (list): a list of indices of the keyframes. keyframe index[segment idx]= key frameindex
        keyframe_boxes_and_labels (list[list[list]]): a list of list which maps from# every video index -> list of segment index -> list of boxes and labels
    ...
    for i, video_idx in enumerate(instances.keys()):
        ...
        #import pdb; pdb.set_trace()
        #vid_seg_map[video_idx] = (count, count + len(instances[video_idx].keys()))#poorvi
        for sec in instances[video_idx].keys():#sec from middle_frame_timestamp
    ...
    # import pdb; pdb.set_trace()
    #pkl.dump(vid_seg_map,open('vid_seg_map.pkl','wb'))#poorvi
