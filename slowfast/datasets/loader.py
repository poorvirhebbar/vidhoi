...
def hoi_collate(batch):
    ...
    Returns:
        (tuple): collated detection data batch.
    """
    # inputs : images, extra_data : meta_data
    ...
    for key in extra_data[0].keys():
        data = [d[key] for d in extra_data]
        if key in ["boxes", "ori_boxes", "obj_classes", "action_labels", "gt_boxes", "proposal_classes", "proposal_scores", "trajectories", "human_poses", "trajectory_boxes", "skeleton_imgs", "trajectory_box_masks", "heatmaps"]:
            # Append idx info to the bboxes before concatenating them.
            if key in ['obj_classes', 'proposal_classes', 'proposal_scores']: # use a mask
                ...
                try:
                    if key in ['skeleton_imgs', 'trajectory_box_masks', 'heatmaps']:
                        bboxes = [
                            np.concatenate(
                                [np.full((data[i].shape[0], 1, data[i].shape[2], data[i].shape[3], data[i].shape[4]), float(i)), data[i]], axis=1
                            )
                            for i in range(len(data))
                        ]
                    #elif key in ['trajectories']:#this part can be a problem if testing st-hoi, as trajectories are sampled here
                    #    bboxes = [
                    #        np.concatenate(
                    #            [np.full((data[i].shape[0], data[i].shape[1], 1), float(i)), data[i]], axis=2
                    #        )
                    #        for i in range(len(data))
                    #    ]
                    elif key in ['trajectory_boxes'] and data[0].shape[-1]==129:
                        # Attention update indexes here
                        # only required when saved feats and trajectory_boxes are used
                        #import pdb; pdb.set_trace()
                        bboxes = [
                            np.concatenate(
                                [np.full((data[i].shape[0], 1), float(i)), data[i][:,1:]], axis=1
                            )
                            for i in range(len(data))
                        ]
                    else:
