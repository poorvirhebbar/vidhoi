...
def get_sequence(center_idx, half_len, sample_rate, num_frames):
    ...
    Args:
        center_idx (int): center frame idx for current clip, sec*fps
        half_len (int): half of the clip length, 32
        sample_rate (int): sampling rate for sampling frames inside of the clip, 2
        num_frames (int): number of expected sampled frames, frame_count_of_video
...
def pack_pathway_output(cfg, frames, trajectories):
    """
    ...
    Args:
        ...
        trajectories (tensor): trajectory bounding boxes of images sampled from the video. The
            dimension is `channel` x `num frames` x 4.
    Returns:
        frame_list (list): list of tensors with the dimension of
            `channel` x `num frames` x `height` x `width`.
        
        trajectories (tensor): tensor with the dimension of
            `channel` x `num frames` x 4.
    """
    if cfg.DATA.REVERSE_INPUT_CHANNEL:
        frames = frames[[2, 1, 0], :, :, :]
    if cfg.MODEL.ARCH in cfg.MODEL.SINGLE_PATHWAY_ARCH:
        frame_list = [frames]
    if cfg.MODEL.ARCH in cfg.MODEL.SINGLE_SAMPLED_PATHWAY_ARCH:
        sampled_frames = torch.index_select(
            frames,
            1,
            torch.linspace(
        ...
        raise NotImplementedError(
            "Model arch {} is not in {}".format(
                cfg.MODEL.ARCH,
                cfg.MODEL.SINGLE_PATHWAY_ARCH + cfg.MODEL.MULTI_PATHWAY_ARCH + cfg.MODEL.SINGLE_SAMPLED_PATHWAY_ARCH,
            )
        )
    return frame_list,trajectories
