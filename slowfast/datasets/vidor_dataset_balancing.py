...
import os
import pandas as pd
from . import vidor_helper_balancing as vidor_helper
...
class Vidor(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        act_vid_map = pd.read_pickle(r'../../../vidor-dataset/vidHOI/train/act_vid_map.pkl')#action: array of videos in which it appears
        vid_act_count_map = pd.read_pickle(r'../../../vidor-dataset/vidHOI/train/vid_act_count_map.pkl')#video: dictionary(action:count....)
        ...
        video_index = video_id_list[video_num] #random video selected from that list

        # dict:map -> {video_idx:{start, end}}
        #vid_act_count_map = pd.read_pickle(r'vid_seg_map.pkl')#video: dictionary(action:count....)


        #segs = self.keyframe_indices[vid_act_count_map[video_idx][start]: vid_act_count_map[video_idx][end]]
        #check vidor_helper on how to get segs line 397
        

        #train_action_freq=pd.read_pickle(r'../../../vidor-dataset/vidHOI/train/train_action_frequency_class.pkl')#action_class: frequency

        
        video_idx, sec_idx, sec, center_idx, orig_video_idx = self._keyframe_indices[video_index] #[idx]
