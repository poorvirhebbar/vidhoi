...
def crop_boxes_fixed(boxes, x_offset, y_offset, size, height, width):
    """
    ...
    boxes = clip_boxes_to_image(boxes, l_side, l_side)
    boxes = boxes * size / l_side
...
def random_crop_list(images, size, pad_size=0, order="CHW", boxes=None, n_imgs=0, USE_SPA_CONF=False):
    ...
    if order == "CHW":
        ...
        y_offset = 0
        if height > size:
            y_offset = int(np.random.randint(0, 2)) #height - size))
        x_offset = 0
        if width > size:
            x_offset = int(np.random.randint(0, 2)) # width - size))
        cropped = [
            image[:, y_offset : y_offset + size, x_offset : x_offset + size]
            for image in images
        ]
...
def center_crop_fixed(size, images, boxes):
    """
    ...
    width = images[0].shape[1]
    if height >= width:
        y_offset = int(math.ceil((height - width) / 2))
        x_offset = 0
        cropped = [image[y_offset : y_offset + width, x_offset : x_offset + width, :]
                    for image in images
                    ]
    if width > height:
        y_offset = 0
        x_offset = int(math.ceil((width - height ) / 2))
        cropped = [image[y_offset : y_offset + height, x_offset : x_offset + height, :]
                    for image in images
                    ]
    ...
    if boxes is not None:
        boxes = [crop_boxes_fixed(proposal, x_offset, y_offset, size, height, width) for proposal in boxes]
    return cropped, boxes
